...
# Script taken from https://github.com/groundnuty/k8s-wait-for
# LICENSE MIT

# If sed is failing, install gnu-sed
...
get_job_state() {
    ...
    get_job_state_output1=$(printf "%s" "$get_job_state_output" | gsed -nr 's#.*/ (0+) .*/.*#\1#p' 2>&1)
    if [ $? -ne 0 ]; then
        echo "$get_job_state_output" >&2
